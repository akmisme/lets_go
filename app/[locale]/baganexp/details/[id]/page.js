...
export default function BaganexpDetail() {
  const { id } = useParams();
  const [bgexp, setbgexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Bg_Exp_List}/${id}`);
  ...
  if (!bgexp) return <p>Loading...</p>;

  // Pick localized values
  const title = bgexp.title[locale];
  const para = bgexp.para[locale];
  const subtitle = bgexp.subtitle[locale];
  const description = bgexp.description[locale];
  const subtitle1 = bgexp.subtitle1[locale];
  const should = bgexp.shouldknow[locale]; // array of strings
