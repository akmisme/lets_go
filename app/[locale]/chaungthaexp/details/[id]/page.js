...
export default function CtexpDetail() {
  const { id } = useParams();
  const [chexp, setchexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Ch_Exp_List}/${id}`);
  ...
  if (!chexp) return <p>Loading...</p>;

  // Pick localized values
  const title = chexp.title[locale];
  const para = chexp.para[locale];
  const subtitle = chexp.subtitle[locale];
  const description = chexp.description[locale];
  const subtitle1 = chexp.subtitle1[locale];
  const should = chexp.shouldknow[locale]; // array of strings
