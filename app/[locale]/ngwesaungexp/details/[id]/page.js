...
export default function BaganexpDetail() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Ng_Exp_List}/${id}`);
  ...
  if (!ngexp) return <p>Loading...</p>;

  // Pick localized values
  const title = ngexp.title[locale];
  const para = ngexp.para[locale];
  const subtitle = ngexp.subtitle[locale];
  const description = ngexp.description[locale];
  const subtitle1 = ngexp.subtitle1[locale];
  const should = ngexp.shouldknow[locale]; // array of strings
