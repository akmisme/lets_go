...
export default function BaganexpDetail() {
  const { id } = useParams();
  const [inleexp, setinleexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Inle_Exp_List}/${id}`);
  ...
  if (!inleexp) return <p>Loading...</p>;

  // Pick localized values
  const title = inleexp.title[locale];
  const para = inleexp.para[locale];
  const subtitle = inleexp.subtitle[locale];
  const description = inleexp.description[locale];
  const subtitle1 = inleexp.subtitle1[locale];
  const should = inleexp.shouldknow[locale]; // array of strings
