...
export default function BaganexpDetail() {
  const { id } = useParams();
  const [kalawexp, setkalawexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Kalaw_Exp_List}/${id}`);
  ...
  if (!kalawexp) return <p>Loading...</p>;

  // Pick localized values
  const title = kalawexp.title[locale];
  const para = kalawexp.para[locale];
  const subtitle = kalawexp.subtitle[locale];
  const description = kalawexp.description[locale];
  const subtitle1 = kalawexp.subtitle1[locale];
  const should = kalawexp.shouldknow[locale]; // array of strings
