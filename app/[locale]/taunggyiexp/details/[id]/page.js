...
export default function TgexpDetail() {
  const { id } = useParams();
  const [tgexp, settgexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Tgi_Exp_List}/${id}`);
  ...
  if (!tgexp) return <p>Loading...</p>;

  // Pick localized values
  const title = tgexp.title[locale];
  const para = tgexp.para[locale];
  const subtitle = tgexp.subtitle[locale];
  const description = tgexp.description[locale];
  const subtitle1 = tgexp.subtitle1[locale];
  const should = tgexp.shouldknow[locale]; // array of strings
