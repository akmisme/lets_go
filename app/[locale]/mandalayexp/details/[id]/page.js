...
export default function MandalayexpDetail() {
  const { id } = useParams();
  const [mdexp, setMdexp] = useState(null);
  const locale = useLocale(); // "en" or "mm"

  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`${ENDPOINT.Mdy_Exp_List}/${id}`);
  ...
  if (!mdexp) return <p>Loading...</p>;

  // Pick localized values
  const title = mdexp.title[locale];
  const para = mdexp.para[locale];
  const subtitle = mdexp.subtitle[locale];
  const description = mdexp.description[locale];
  const subtitle1 = mdexp.subtitle1[locale];
  const should = mdexp.shouldknow[locale]; // array of strings
